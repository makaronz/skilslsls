...
Testing Guide for Vue Framework — comprehensive strategies, implementation details, and best practices.

---

## Introduction

This reference provides detailed, actionable guidance on testing guide within the context of Vue Framework. Use this document as a deep-dive resource when implementing the strategies outlined in the main SKILL.md file.
...
|-----------|------|-------|
| Unit | Vitest + Vue Test Utils | Individual components |
| Integration | Vitest + mounting | Component interactions |
| E2E | Cypress or Playwright | Full user flows |
| Snapshot | Vitest snapshot | Render output |
...
function withSetup(composable: () => any) {
  let result;
  createApp({ setup() { result = composable(); return () => {}; }}).mount(document.createElement('div'));
  return result;
}
```
...
- Create a fresh Pinia instance for each test: `setActivePinia(createPinia())`
- Test actions by calling them and asserting state changes
- Mock API calls in store actions
