...
Performance Optimization for Vue Framework — comprehensive strategies, implementation details, and best practices.

---

## Introduction

This reference provides detailed, actionable guidance on performance optimization within the context of Vue Framework. Use this document as a deep-dive resource when implementing the strategies outlined in the main SKILL.md file.
...
- **`v-memo`**: Memoize template subtrees based on dependencies
- **`shallowRef`/`shallowReactive`**: Avoid deep reactivity overhead
- **`defineAsyncComponent`**: Lazy-load heavy components
- **`KeepAlive`**: Cache component instances instead of destroying

#### Rendering Performance

| Technique | Impact | When to Use |
|-----------|--------|-------------|
| Virtual scrolling | High | Long lists (>100 items) |
| Lazy loading | High | Below-fold content |
| Computed caching | Medium | Expensive derivations |
| Key attribute | Medium | List rendering |
| Functional components | Low | Pure display components |

#### Bundle Size Optimization

- Tree-shake unused Vue features with build tool configuration
...
- Use Vue DevTools performance tab to identify slow components
- Monitor component render count and duration
- Profile with Chrome DevTools for bottlenecks
- Set performance budgets and track bundle size in CI
