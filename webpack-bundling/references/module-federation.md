...
Module Federation for Webpack Bundling — comprehensive strategies, implementation details, and best practices.

---

## Introduction

This reference provides detailed, actionable guidance on module federation within the context of Webpack Bundling. Use this document as a deep-dive resource when implementing the strategies outlined in the main SKILL.md file.

## Webpack Module Federation

### Module Federation Overview

Module Federation enables loading remote modules at runtime from separate builds:
...
|---------|------------|----------|
| Host-Remote | One host loads multiple remotes | Micro-frontends |
| Bidirectional | Apps load from each other | Shared component libraries |
| Dynamic Remote | Runtime URL resolution | Multi-tenant, A/B testing |

### Shared Dependencies

- Mark common dependencies as `shared` to avoid duplication
- Use `singleton: true` for packages that must be single-instance (React)
- Set `requiredVersion` for compatibility enforcement
- Use `eager: true` sparingly — only for the host application
