...
Troubleshooting for Git Version Control — comprehensive strategies, implementation details, and best practices.

---

## Introduction

This reference provides detailed, actionable guidance on troubleshooting within the context of Git Version Control. Use this document as a deep-dive resource when implementing the strategies outlined in the main SKILL.md file.
...
|---------|-------|----------|
| Merge conflicts | Overlapping changes | Resolve conflicts, then `git add` + `git commit` |
| Detached HEAD | Checkout to commit/tag | `git checkout main` or `git switch main` |
| Accidentally committed to wrong branch | Wrong branch active | Cherry-pick to correct branch, reset |
| Need to undo last commit | Premature commit | `git reset --soft HEAD~1` |
| Committed sensitive data | Secret in repo | BFG Repo-Cleaner or `git filter-branch` |
| Large repo, slow clone | Full history | `git clone --depth 1` (shallow) |
...
- `git reflog` — find any previous HEAD position
- `git fsck --lost-found` — recover dangling commits and blobs
- `git stash list` — check if work was stashed
- IDE local history — last resort for unsaved changes

### Performance Issues

- Large files: Use Git LFS for binary files >10MB
- Slow operations: Run `git gc` and `git prune`
- Large repos: Use sparse checkout for subset of files
