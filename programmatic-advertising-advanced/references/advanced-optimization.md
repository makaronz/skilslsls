...
Advanced Optimization for Programmatic Advertising Advanced — comprehensive strategies, implementation details, and best practices.

---

## Introduction

This reference provides detailed, actionable guidance on advanced optimization within the context of Programmatic Advertising Advanced. Use this document as a deep-dive resource when implementing the strategies outlined in the main SKILL.md file.

## Programmatic Platform Overview

Understanding Programmatic's advertising ecosystem is essential for effective advanced optimization. The platform offers unique capabilities that differentiate it from other advertising channels.
...
Follow a structured approach to campaign optimization:

1. **Diagnose**: Identify which metrics are underperforming
2. **Hypothesize**: Form a theory about the root cause
3. **Test**: Implement a controlled change
4. **Measure**: Allow sufficient data collection (minimum 3–7 days)
5. **Scale**: Apply winning strategies across campaigns

### Bid Optimization

| Strategy | When to Use | Pros | Cons |
|----------|------------|------|------|
| Manual CPC/CPM | Testing phase | Full control | Time-intensive |
| Target CPA | Steady conversion data | Predictable costs | Needs history |
| Maximize conversions | Scaling phase | Algorithm-driven | Less control |
| Target ROAS | E-commerce with value data | Revenue-focused | Requires value tracking |
| Lowest cost | Budget-constrained | Efficient spend | Unpredictable CPA |

### Budget Allocation

- **70/20/10 rule**: 70% proven campaigns, 20% scaling, 10% testing
- **Daily vs. lifetime budgets**: Use daily for ongoing campaigns, lifetime for fixed-date promotions
- **Dayparting**: Allocate budget to high-performing hours and days
- **Geographic weighting**: Shift budget toward higher-performing regions

### Performance Benchmarks

| Metric | Good | Excellent | Action if Below |
|--------|------|-----------|----------------|
| CTR | >1.0% | >2.0% | Refresh creatives, refine targeting |
| Conversion Rate | >2.0% | >5.0% | Optimize landing page, check audience |
| CPA | At target | Below target | Scale budget |
| ROAS | >3x | >5x | Increase investment |
| Quality/Relevance Score | >6/10 | >8/10 | Improve ad-audience alignment |

### Scaling Strategies

- **Horizontal scaling**: Duplicate winning ad sets with new audiences
- **Vertical scaling**: Increase budget by 20–30% every 3–5 days
- **Creative scaling**: Launch new creatives based on winning angles
- **Geographic expansion**: Test new markets with proven campaigns
